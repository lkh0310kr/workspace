...
app as a child process (`apps/workspace/src/main/worldEngine.ts`, mirroring how
`apps/workspace/src/main/pty.ts` manages a shell) — **not** embedded as a pane.
...
For Workspace packaging, build the engine first, then `cd apps/workspace && npm run build:win` (see `apps/workspace/scripts/build-world-engine-and-electron-win.ps1`).
...
lives at `apps/workspace/test-fixtures/world-engine-demo/`.
...
`apps/workspace/test-fixtures/world-engine-physics-demo/`.

A **Rhai chase** example (scripted kinematic entity) lives at
`apps/workspace/test-fixtures/world-engine-chase-demo/`.
...
`apps/workspace/test-fixtures/world-engine-joints-demo/`.
...
scene — real example at `apps/workspace/test-fixtures/world-engine-mesh-demo/`:
