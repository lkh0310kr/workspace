...
- [ ] **검증 대기: Claude Code rate-limit 진행바** ("Claude Code <bar> N% used 4h 52m" — Orca UI 참고해서 구현) — 위 Gatekeeper 원인이 이제 밝혀졌으니(tmux 안 거치는 터미널에서 빌드하면 됨), 더 이상 막혀있지 않음. 다음에 열 때 바로 검증 가능. 구현 내용:
