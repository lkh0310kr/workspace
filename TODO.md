다음 작업 (순서대로):
1. **Vector Editor 라이브 QA** — 아래 "Vector Editor QA (M1-M5 전체)" 항목을 직접 GUI에서 돌려보고 버그/불편한 점 알려주기. 이게 끝나야 다음 단계로 넘어감 (한 페인을 완전히 완성하고 나서 다음 페인/기능으로 — 세션 내내 지킨 원칙).
2. QA에서 나온 버그 수정.
3. QA 통과하면 그 다음은 둘 중 하나 (아직 미확정, 그때 다시 정하기):
   - `docs/architecture/10-creative-panes-ux-roadmap.md`의 Tier 1(pan/zoom, marquee select, z-order, flip)부터 순서대로 Vector Editor UX 개선
...
- [ ] Vector Editor QA (M1-M5 전체) — 새 패널 "Vector"로 탭 생성 →
...
- [ ] Vector: 뷰포트 pan (spacebar+drag)
- [ ] Vector: 뷰포트 zoom (wheel/트랙패드 핀치)
- [ ] Vector: 줌 리셋 버튼/단축키
- [ ] Vector: 줌-투-핏(전체 보기) 버튼
- [ ] Vector: 줌-투-셀렉션(선택 영역에 맞춤) 버튼
- [ ] Vector: 현재 줌 배율 표시 (예: 100%)
- [ ] Vector: 줌 상태를 어디에 둘지 결정 (문서에 저장 vs 세션 로컬)
- [ ] Vector: 빈 캔버스 드래그 시 marquee(고무줄) 선택 사각형
- [ ] Vector: marquee와 교차하는 오브젝트 전부 다중 선택
- [ ] Vector: Shift+marquee로 기존 선택에 추가
- [ ] Vector: marquee가 그룹 내부까지 뚫고 들어가지 않고 그룹 전체를 선택하는지 (기존 클릭 규칙과 일관성)
- [ ] Vector: Bring to Front 액션+단축키
- [ ] Vector: Bring Forward 액션+단축키
- [ ] Vector: Send Backward 액션+단축키
- [ ] Vector: Send to Back 액션+단축키
- [ ] Vector: Flip Horizontal 액션+단축키
- [ ] Vector: Flip Vertical 액션+단축키
