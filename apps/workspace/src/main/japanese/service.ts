...
export function getJapaneseDbStatus(): JapaneseDbStatus {
  ...
  if (!ready) {
    if (connectError?.includes("NODE_MODULE_VERSION")) {
      loadMessage =
        "SQLite 네이티브 모듈이 Electron과 맞지 않습니다. 터미널에서 cd apps/workspace && npm install && npm run rebuild:native 실행 후 앱을 재시작하세요.";
    } else if (connectError) {
      loadMessage = `사전 DB 연결 실패: ${connectError}`;
    } else {
      const existing = probes.filter((probe) => probe.exists);
      if (existing.length === 0) {
        loadMessage = `dictionary.db를 찾을 수 없습니다. apps/workspace/에서 npm run japanese:import:fixtures 실행 후 Reload 하세요.`;
