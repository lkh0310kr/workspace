...
describe("worldEngineEmbedModuleCandidates", () => {
  ...
  it("includes dev release and debug paths on macOS", () => {
    ...
    expect(candidates).toContain(
      "/app/native/world-engine-electron-embed/target/release/world_engine_electron_embed.node",
    );
    expect(candidates).toContain(
      "/app/native/world-engine-electron-embed/target/debug/libworld_engine_electron_embed.dylib",
    );
  });
});
