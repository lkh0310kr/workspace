...
export function worldEngineBinaryCandidates(options: WorldEngineBinaryOptions = {}): string[] {
  ...
  if (electronAppPath) {
    const base = path.join(electronAppPath, "..", "native", "world-engine-qt-shell", "target");
...
function buildHint(): string {
  if (isWsl()) {
    return (
      ...
      "  cd native\\world-engine-qt-shell && .\\scripts\\build-windows.ps1 -Release\n" +
      "See docs/windows-build.md"
    );
  }
  if (process.platform === "win32") {
    return "Install Qt 6 MSVC, then: cd native\\world-engine-qt-shell && .\\scripts\\build-windows.ps1 -Release\nSee docs/windows-build.md";
  }
  if (process.platform === "darwin") {
    return "brew install qt, then: cd native/world-engine-qt-shell && cargo build";
  }
  return "Install Qt 6 (qt6-base-dev), then: cd native/world-engine-qt-shell && cargo build";
}
