...
// Why: reparenting a terminal container during a flexlayout split/move
// resets the viewport scroll position (browser clears scrollTop on DOM
// move). This schedules a two-phase restore: an early double-rAF (~32ms)
// to minimise the visible flash, plus a 200ms authoritative restore that
// also clears the scroll lock.
//
// The optional reattachWebgl callback refreshes the WebGL addon after the
// DOM has settled, mirroring Orca's split-reparent WebGL rebuild.
export function scheduleSplitScrollRestore(
  ...
  const settleTimerId = setTimeout(() => {
    ...
    // and a full refresh here clobbers an in-progress draw. WebGL reattach
    // also refreshes, so defer it until the TUI exits the alternate buffer.
    // Alt-screen has no scrollback, so scroll restore has nothing to do.
