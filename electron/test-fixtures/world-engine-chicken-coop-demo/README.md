...
고정 카메라로 위에서 보면 색상 매트로 구역이 구분됩니다.

```
                    N (fence)
         ┌─────────────────────────────┐
         │  [MGMT] 파랑 — 사료·물      │
    W    │      feed / water / hopper   │    E
         │                             │
         │         rain_shelter        │
         │                             │
         │              [LIVE] 녹색     │
...
| 매트 | 색 | PKMS 의미 |
|------|-----|-----------|
| `zone_mgmt` | 파랑 (NW) | 관리 구역 — 사료·물 |
| `zone_living` | 녹색 (SE) | 생활 구역 — 둥지·횃대·비막이 |
| `zone_gate` | 노랑 (S) | 출입·알 수거 |
...
카메라는 탑다운 고정 뷰입니다 (`position: [0, 22, 0]`).
