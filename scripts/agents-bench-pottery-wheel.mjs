#!/usr/bin/env node
/**
 * B7 bench: rebuild bench-pottery-wheel assembly and print timing JSON.
 *
 * Vibe-CAD loop (manual viewer leg):
 *   1. Open models/bench-pottery-wheel/STEP/assembly.step in Workspace
 *   2. Edit src/lib/dims.py (e.g. WHEEL_D)
 *   3. npm run agents:bench:pottery-wheel
 *   4. Viewer soft-reloads on fs:changed when assembly.step mtime updates
 */
...
const SRC = join(ROOT, "models", "bench-pottery-wheel", "src");
const ASSEMBLY = join(SRC, "assembly.py");
const ASSEMBLY_STEP = join(ROOT, "models", "bench-pottery-wheel", "STEP", "assembly.step");
...
const report = {
  ...
  outputs: {
    assembly: "models/bench-pottery-wheel/STEP/assembly.step",
    dims: "models/bench-pottery-wheel/src/lib/dims.py",
  },
  viewer_hint:
